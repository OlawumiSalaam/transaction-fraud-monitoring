...
Responsibilities (Addendum §4): write an append-only per-case record to
...
timestamps) is assembled by the milestones that produce those artifacts (M4–M8)
and passed in here.
...
* Append-only. This class exposes ``append`` and nothing else — no update, no
  ...
  and production revokes UPDATE/DELETE at the database role level (NFR-7).
* A decision must be reconstructable from ``audit_log`` alone (FR-20, NFR-3).
"""
