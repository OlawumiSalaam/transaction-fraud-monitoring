"""Rule Engine: deterministic if-then evaluation over the shared substrate (FR-6).
...
independently and both preserved as evidence (Addendum §4; Principle: Layer
Separation). Rule outputs are kept visibly distinct from model outputs.

This matters especially now that the M2 scorer is gate-ineligible (excluded from the
operational path under FR-4): the deterministic rule engine continues to operate as
designed and is the case's primary evidence source — graceful degradation, not a
workaround.

Contract (Addendum §4 — Rule Engine):
  In:  the feature substrate for a transaction (``FeatureVector``).
  Out: ``[RuleHit]`` (each carrying the fields that made it fire).

Spec references: FR-6, FR-7; Addendum §4; Release Plan M3; Principle: Layer Separation.
"""
...
class RuleEngine:
    """Evaluate the enabled deterministic rules (FR-6). Independent of the score.
